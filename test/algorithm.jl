@testset "Solving algorithm tests" begin
    ...
    @testset " Validation Test || ATMC || basic solve || examples/nlp3.jl (40s)" begin
        ...
        ans[2] = [1000.0,1137.29,1207.67,1295.76,1313.48,1352.39,1413.05,1606.7,1739.7,3609.97,10000.0]
        ans[3] = [1000.0,1352.05,2282.04,2859.97,3365.52,4221.02,4570.98,4816.66,5385.71,5709.08,7359.97,10000.0]
        ans[4] = [10.0,16.5493,126.38,160.694,173.523,185.647,210.658,226.308,429.518,1000.0]
        ans[5] = [10.0,48.1012,141.794,242.271,278.835,284.286,296.91,304.084,340.71,366.021,389.294,543.101,1000.0]
        ans[6] = [10.0,164.701,185.971,212.667,213.15,227.327,241.02,242.678,278.115,392.442,465.482,1000.0]
        ans[7] = [10.0,38.9165,132.135,212.799,244.965,269.072,281.43,296.898,300.01,306.84,336.549,379.635,533.917,1000.0]
        ans[8] = [10.0,148.101,241.794,342.271,378.835,384.286,396.91,404.084,440.71,466.021,489.294,643.101,1000.0]

        for i in 1:8
            for j in 1:length(ans[i])
                @show m.internalModel.discretization[i]
                @show ans[i]
                @test length(ans[i]) == length(m.internalModel.discretization[i])
                @test isapprox(ans[i][j], m.internalModel.discretization[i][j]; atol=1e-3)
            end
        end
    end
