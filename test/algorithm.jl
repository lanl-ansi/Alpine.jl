...
@testset "Solving algorithm tests :: Bin-Lin Solves" begin
    ...
    @testset "Operator :: bmpl && linbin && binprod solve test II" begin
        ...
        @test m.internalModel.nonlinear_terms[Expr[:(x[7]), :(x[8])]][:nonlinear_type] == :multilinear
        @test m.internalModel.nonlinear_terms[Expr[:(x[8]), :(x[9])]][:nonlinear_type] == :multilinear
        @test m.internalModel.nonlinear_terms[Expr[:(x[9]), :(x[10])]][:nonlinear_type] == :multilinear
        @test m.internalModel.nonlinear_terms[Expr[:(x[10]), :(x[6])]][:nonlinear_type] == :multilinear
