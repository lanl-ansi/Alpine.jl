...
function logging_summary(m::Optimizer)
    if Alp.get_option(m, :log_level) > 0
        ...
        println(
            "  # Potential variables for partitioning = ",
            length(m.disc_vars),
        )
        ...
        (Alp.get_option(m, :convhull_ebd)) &&
            println("  Using convhull_ebd formulation")
        (Alp.get_option(m, :convhull_ebd)) && println(
            "  Encoding method = $(Alp.get_option(m, :convhull_ebd_encode))",
        )
        (Alp.get_option(m, :convhull_ebd)) && println(
...
function summary_status(m::Optimizer)
    ...
    if m.detected_bound && m.detected_feasible_solution
        m.alpine_status =
            m.best_rel_gap > Alp.get_option(m, :rel_gap) ? MOI.OTHER_LIMIT :
            MOI.OPTIMAL
