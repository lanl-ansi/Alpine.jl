...
function eval_opt_gap(m::Optimizer, lower_bound::Number, upper_bound::Number)
    ...
    elseif (lower_bound - upper_bound) > 1E-5
        error(
            "Lower bound cannot cross the upper bound in optimality gap evaluation",
        )
    ...
    elseif isinf(lower_bound) || isinf(upper_bound)
        ...
        if isapprox(upper_bound, 0.0; atol = tol) # zero upper bound case
            ...
            m.best_rel_gap =
                abs(upper_bound - lower_bound) / (tol + abs(upper_bound))
        end
    end
...
function check_solution_history(m::Optimizer, ind::Int)
    ...
    (m.logs[:n_iter] < Alp.get_option(m, :disc_consecutive_forbid)) &&
        return false
    ...
    Alp.get_option(m, :log_level) > 99 && println(
        "Consecutive bounding solution on VAR$(ind) obtained. Diverting...",
    )
...
function fix_domains(m::Optimizer; discrete_sol = nothing, use_orig = false)
    ...
    for i in 1:m.num_var_orig
        if i in m.disc_vars && m.var_type[i] == :Cont
            ...
            for j in 1:PCnt
                if point >=
                   (m.discretization[i][j] - Alp.get_option(m, :tol)) &&
...
function get_active_partition_idx(
    ...
    for j in 1:length(discretization[idx])-1
        if val > discretization[idx][j] - tol &&
           val < discretization[idx][j+1] + tol
            return j
        end
    end
...
function initialize_solution_pool(m::Optimizer, cnt::Int)
    ...
    s[:vars] =
        [i for i in m.candidate_disc_vars if length(m.discretization[i]) > 2]
...
function ncvar_collect_arcs(m::Optimizer, nodes::Vector)
    ...
    for k in keys(m.nonconvex_terms)
        ...
        elseif m.nonconvex_terms[k][:nonlinear_type] in
               [:BININT, :BINLIN, :BINPROD]
            continue
        else
            error(
...
function weighted_min_vertex_cover(m::Optimizer, distance::Dict)
    ...
    for i in m.candidate_disc_vars
        ...
        (Alp.get_option(m, :log_level) > 100) && println(
            "VAR$(i) WEIGHT -> $(weights[i]) ||| DISTANCE -> $(distance[i])",
        )
    end
    ...
    Alp.get_option(m, :log_level) >= 99 && println(
        "UPDATED DISC-VAR COUNT = $(length(m.disc_vars)) : $(m.disc_vars)",
    )
...
function fetch_mip_solver_identifier(m::Optimizer; override = "")
    isempty(override) ? solverstring = string(Alp.get_option(m, :mip_solver)) :
    solverstring = override
    ...
    elseif occursin("Xpress", solverstring)
        ...
        error(
            "Unsupported MIP solver $solverstring; use a Alpine-supported MIP solver",
        )
    end
...
function fetch_nlp_solver_identifier(m::Optimizer; override = "")
    isempty(override) ? solverstring = string(Alp.get_option(m, :nlp_solver)) :
    solverstring = override
    ...
    elseif occursin("NLopt", solverstring)
        ...
        error(
            "Unsupported NLP local solver $solverstring; use a Alpine-supported NLP local solver",
        )
    end
...
function fetch_minlp_solver_identifier(m::Optimizer; override = "")
    ...
    isempty(override) ?
    solverstring = string(Alp.get_option(m, :minlp_solver)) :
    solverstring = override
    ...
    elseif occursin("Juniper", solverstring)
        ...
        error(
            "Unsupported MINLP local solver $solverstring; use an Alpine-supported MINLP local solver (Juniper.jl for example)",
        )
    end
...
function resolve_lifted_var_value(m::Optimizer, sol_vec::Array)
    ...
    sol_vec =
        [sol_vec; fill(NaN, m.num_var_linear_mip + m.num_var_nonlinear_mip)]
    ...
    for i in 1:length(m.term_seq)
        ...
        elseif haskey(m.linear_terms, k)
            ...
            sol_vec[lvar_idx] =
                m.linear_terms[k][:evaluator](m.linear_terms[k], sol_vec)
        else
            error(
                "[RARE] Found homeless term key $(k) during bound resolution.",
            )
        end
    end
