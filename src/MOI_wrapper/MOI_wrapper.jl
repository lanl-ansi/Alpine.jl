...
function MOI.set(
    ...
    model.best_sol[vi.value] =
        model.initial_warmval[vi.value] = something(value, 0.0)
    return
end
...
function MOI.add_constraint(
    ...
    push!(
        ...
        MOI.NLPBoundsPair(
            something(_lower(set), -Inf),
            something(_upper(set), Inf),
        ),
    )
    iszero(f.constant) || throw(
        MOI.ScalarFunctionConstantNotZero{Float64,typeof(f),typeof(set)}(
            f.constant,
        ),
    )
    push!(model.lin_quad_constraints, (copy(f), copy(set)))
    push!(
        model.constr_expr_orig,
        _constraint_expr(_moi_function_to_expr(f), set),
    )
