function amp_post_convhull(m::Optimizer; kwargs...)
    ...
    for k in keys(m.nonconvex_terms)
        ...
        elseif nl_type == :INTPROD && !m.nonconvex_terms[k][:convexified]
            error(
                "Integer features are OFF. No support for INTPROD at this moment.",
            )
        elseif nl_type == :INTLIN && !m.nonconvex_terms[k][:convexified]
            error(
                "Integer features are OFF. No support for INTLIN at this moment.",
            )
...
function amp_convexify_multilinear(
    ...
    ml_indices, dim, extreme_point_cnt =
        Alp.amp_convhull_prepare(m, discretization, k)   # convert key to easy read mode
...
function amp_convexify_binlin(m::Optimizer, k::Any, β::Dict)
    ...
    bin_idx =
        [i for i in m.nonconvex_terms[k][:var_idxs] if m.var_type[i] == :Bin]
    cont_idx =
        [i for i in m.nonconvex_terms[k][:var_idxs] if m.var_type[i] != :Bin]
...
function amp_convexify_binprod(m::Optimizer, k::Any, β::Dict)
    ...
    x = [
        _index_to_variable_ref(m.model_mip, i) for
        i in m.nonconvex_terms[k][:var_idxs]
    ]
    for i in x
...
function amp_convhull_prepare(
    ...
    if isa(nonlinear_key, Vector)
        for var in nonlinear_key        # This output regulates the sequence of how composing variable should be arranged
            ...
            m.var_type[var.args[2]] in [:Cont, :Int] &&
                push!(counted_var, var.args[2])
        end
...
function amp_no_good_cut_α(m::Optimizer, α::Dict)
    ...
    for i in 1:m.bound_sol_pool[:cnt]
        (m.bound_sol_pool[:stat][i] == :Cutoff) &&
            (m.bound_sol_pool[:stat][i] = :Alive)
        if m.best_obj < m.bound_sol_pool[:obj][i] &&
           m.bound_sol_pool[:stat][i] == :Alive
            ...
            JuMP.@constraint(
                m.model_mip,
                sum(
                    α[v][m.bound_sol_pool[:disc][i][v]] for v in no_good_idxs
                ) <= no_good_size
            )
    ...
    if m.bound_sol_pool[:cnt] >= 2 # can only warm-start the problem when pool is large enough
        ...
        if ws_idx > 0 # If a warm starter is found
            for v in m.bound_sol_pool[:vars]
                ...
                active_j = Alp.get_active_partition_idx(
                    d,
                    m.bound_sol_pool[:sol][ws_idx][v],
                    v,
                )
                for j in 1:partition_cnt
                    j == active_j ? set_start_value(α[v][j], 1.0) :
                    set_start_value(α[v][j], 0.0)
                end
            end
...
function amp_post_convhull_constrs(
    ...
    for (cnt, i) in enumerate(indices)
        ...
        if m.var_type[i] == :Cont
            ...
            error(
                "EXCEPTION: unexpected variable type during integer related realxation",
            )
        end
        sliced_indices = [
            collect_indices(λ[indices][:indices], cnt, [k], dim) for
            k in 1:l_cnt
...
function amp_post_convhull_constrs(
    ...
    if Alp.get_option(m, :convhull_ebd) && partition_cnt > 2
        ...
        for i in 1:YCnt
            JuMP.@constraint(
                m.model_mip,
                sum(λ[monomial_idx][:vars][collect(ebd_map[i])]) <=
                α[monomial_idx][i]
            )
            JuMP.@constraint(
                m.model_mip,
...
function amp_post_inequalities_cont(
    ...
    if Alp.get_option(m, :convhull_formulation) == "sos2" &&
       ...
       partition_cnt > 2
        ...
        for i in 1:YCnt
            ...
            JuMP.@constraint(
                m.model_mip,
                sum(λ[ml_indices][:vars][n_sliced_indices]) <=
                1 - α[var_ind][i]
            )
        end
    ...
    if Alp.get_option(m, :convhull_formulation) == "sos2"
        for j in 1:lambda_cnt
            sliced_indices =
                collect_indices(λ[ml_indices][:indices], cnt, [j], dim)
            if (j == 1)
                JuMP.@constraint(
                    m.model_mip,
    ...
    elseif Alp.get_option(m, :convhull_formulation) == "facet"
        for j in 1:(partition_cnt-1) # Constraint cluster of α >= f(λ)
            sliced_indices =
                collect_indices(λ[ml_indices][:indices], cnt, [1:j;], dim)
            JuMP.@constraint(
                m.model_mip,
                sum(α[var_ind][1:j]) >=
                sum(λ[ml_indices][:vars][sliced_indices])
        ...
        error(
            "Must indicate a choice of convex hull formulation. ?(sos2, facet)",
        )
    end
...
function amp_post_λ_upperbound(
    ...
    if level == length(indices)
        ...
        sliced_indices = Set(
            collect_indices(λ[indices][:indices], 1, [reg[1]; reg[1] + 1], dim),
        )
...
function collect_indices(
    l::Array,
    fixed_dim::Int,
    fixed_partition::Array,
    dim::Tuple,
)
    k = 0
    indices = Vector{Int}(
        undef,
        Int(prod(dim) / dim[fixed_dim] * length(fixed_partition)),
    )
    for i in 1:prod(dim)
