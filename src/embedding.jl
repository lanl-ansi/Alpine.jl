...
function embedding_map(λCnt::Int, encoding::Any = ebd_gray, ibs::Bool = false)
    ...
    !is_compatible_encoding(H) &&
        error("Encodign method is not SOS-2 compatible...")
...
function resolve_encoding_key(encoding::Any)
    ...
    return error(
        "Must specify a encoding method when using convhull_ebd formulation",
    )
end
