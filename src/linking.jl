function add_linking_constraints(m::Optimizer, λ::Dict)
    ...
    link_info = Dict(link_var_idx=>filter(r->issubset(link_var_idx, r), keys(λ)) 
        for deg in 2:maxdeg-1 for link_var_idx in Combinatorics.combinations(all_vars_idx, deg))
    ...
    for (link_var_idx, list_multi_idx) in link_info, (i, multi_idx) in enumerate(list_multi_idx)
