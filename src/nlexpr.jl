...
function expr_parsing(m::Optimizer)
    ...
    if !is_strucural
        m.bounding_obj_expr_mip =
            Alp.expr_term_parsing(m.bounding_obj_expr_mip, 0, m)
    ...
    for i in 1:m.num_constr_orig
        is_strucural =
            Alp.expr_constr_parsing(m.bounding_constr_expr_mip[i], m, i)
        if !is_strucural
...
function expr_conversion(m::Optimizer)
    ...
    Alp.get_option(m, :log_level) > 199 &&
        println("lifted ::", m.bounding_obj_expr_mip)
    Alp.get_option(m, :log_level) > 199 &&
        println("coeffs ::", m.bounding_obj_mip[:coefs])
    Alp.get_option(m, :log_level) > 199 &&
        println("vars ::", m.bounding_obj_mip[:vars])
    Alp.get_option(m, :log_level) > 199 &&
        println("sense ::", m.bounding_obj_mip[:sense])
    Alp.get_option(m, :log_level) > 199 &&
        println("rhs ::", m.bounding_obj_mip[:rhs])
    ...
    for i in 1:m.num_constr_orig
        ...
        Alp.get_option(m, :log_level) > 199 &&
            println("type :: ", m.constr_structure[i])
        Alp.get_option(m, :log_level) > 199 &&
...
function expr_finalized(m::Optimizer)
    ...
    m.num_constr_convex =
        length([i for i in m.constr_structure if i == :convex])
...
function isa_variable_index(expr::Expr)
    return length(expr.args == 2) &&
           expr.args[2] == :(MathOptInterface.VariableIndex)
end
...
function expr_strip_const(expr, subs = [], rhs = 0.0)
    ...
    for i in 2:length(expr.args)
        if (
            ...
            (expr.args[1] == :-) &&
                ((i == 2) ? rhs -= expr.args[i] : rhs += expr.args[i])
...
function expr_linear_to_affine(expr)
    ...
    elseif expr.head == :ref  # For single variable objective expression
        ...
        lhscoeff, lhsvars, rhs, non, non =
            Alp.traverse_expr_linear_to_affine(expr)
        affdict[:sense] = nothing
    end
...
function traverse_expr_linear_to_affine(
    ...
    for i in start_pos:length(expr.args)
        lhscoeff, lhsvars, rhs, bufferVal, bufferVar =
            traverse_expr_linear_to_affine(
                expr.args[i],
                lhscoeffs,
                lhsvars,
                rhs,
            ...
            if length(expr.args[i].args) == 2 && expr.args[i].head == :call
                if expr.args[i].args[1] == :- # Treatment for :(-x), replace with :(x*-1)
                    ...
                    elseif expr.args[1] in [:+, :-, :(==), :(<=), :(>=)]
                        ...
                        error(
                            "Unexpected operator $(expr.args[i]) during resolving sign",
                        )
                    end
    ...
    if args[1] in [:*, :+, :-]
        ...
        for i in 2:length(args)
            if isa(args[i], Float64) || isa(args[i], Int)
                ...
                val = eval(args[1])(
                    val,
                    eval(reverter[(i==2)&&(args[1]==:-)])(args[i]),
                ) # Revert the first sigh if (-)
    ...
    if (expr.head == :call && expr.args[1] in [:+, :-])
        ...
        for i in 2:length(expr.args)
            ...
            elseif (
                ...
                expr_tmp =
                    Expr(:call, :^, expr_i.args[2].args[3], expr_i.args[3])
                if ((expr.args[1] == :-) && (i > 2)) || (ind == 1)
                    push!(
                        expr_array,
                        Expr(
                            :call,
                            :*,
                            -expr_i.args[2].args[2]^expr_i.args[3],
                            expr_tmp,
                        ),
                    )
                else
