# During tree flattening, these values are rquired to deal with multiple coefficient entries
...
"""
function _pod_expr_t2y(xdim::Int, T::[], t2y=Dict(); kwargs...)
...
function _pod_expr_lift(h, t2y::Dict; kwargs...)
	...
	elseif h.args[1] == :* # with assumption :: bilinear, limited () usage
		...
		if length(refs) == length(h.args) - 1  # (*)->[(x),(x)] ==> (*)->[(1),(y)]
			push!(h.args, 1.0)
			# TODO : consider crush this tree later
		end
	...
	elseif args[1] in [:*, :+, :-]
		...
		for i in 2:length(args)
			if isa(args[i],Float64)
				# valinit += (i==2) # Not sure why this is still here
