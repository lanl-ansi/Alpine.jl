...
function expr_term_parsing(
    ...
    for node in expr.args
        ...
        elseif node.head == :call
            expr.args[cnt] =
                Alp.expr_term_parsing(node, constr_id, m, level + 1)
...
"""
function detect_nonconvex_terms(
    expr::Any,
    constr_id::Int,
    m::Optimizer;
    kwargs...,
...
function store_nonconvex_term(
    ...
    m.nonconvex_terms[nl_key] = Dict(
        ...
        :y_type => Alp.resolve_lifted_var_type(
            [m.var_type[k] for k in var_idxs],
            operator,
        ),
        :var_idxs => var_idxs,
        :ref => nl_key,
...
function lift_nonconvex_term(m::Optimizer, nl_key, constr_id::Int, scalar = 1.0)
    ...
    if scalar == 1.0
        ...
        return Expr(
            :call,
            :*,
            m.nonconvex_terms[nl_key][:lifted_var_ref],
            scalar,
        )
    end
end
...
function detect_linear_term(expr::Any, constr_id::Int, m::Optimizer)
    ...
    if expr.args[1] in [:+, :-]
        ...
        for i in 2:length(expr.args)  # TODO: Consider recursive operation
            ...
            if (expr.args[i].head == :ref) && isa(expr.args[i].args[2], Int)
                ...
                push!(
                    coef_var,
                    (coef_fetch[expr.args[1]], expr.args[i].args[2]),
                )
                continue
            end
            ...
            if expr.args[i].head == :call &&
               ...
               length(expr.args[i].args) == 3
                sub_coef = [
                    j for j in expr.args[i].args if
                    (isa(j, Int) || isa(j, Float64))
                ]
                sub_vars = [
                ...
                (length(sub_coef) != 1 || length(sub_vars) != 1) &&
                    return false, expr
                (i == 2) ? push!(coef_var, (1.0 * sub_coef[1], sub_vars[1])) :
                push!(
                    coef_var,
                    (coef_fetch[expr.args[1]] * sub_coef[1], sub_vars[1]),
                )
                continue
            end
            ...
            if expr.args[i].head == :call &&
               ...
               length(expr.args[i].args) == 2 # resolve -(2) or -(x) terms
                expr.args[i].args[1] == :+ ? sub_coef = [1.0] :
                sub_coef = [-1.0]
                sub_vars = [
                    j.args[2] for j in expr.args[i].args if
                    ((:head in fieldnames(typeof(j))) && j.head == :ref)
        ...
        term_key in keys(m.linear_terms) ||
            Alp.store_linear_term(m, term_key, expr)
    ...
    elseif expr.args[1] in [:*] && length(expr.args) == 3
        ...
        sub_vars = [
            i.args[2] for i in expr.args if
            ((:head in fieldnames(typeof(i))) && i.head == :ref)
        ]
        (isempty(sub_coef) || isempty(sub_vars)) && return false, expr
...
function basic_linear_bounds(m::Optimizer, k::Any, linear_terms = nothing)
    linear_terms === nothing ? linear_terms = m.linear_terms :
    linear_term = linear_terms
...
function basic_linear_bounds(m::Optimizer, k::Any, d::Dict)
    ...
    for j in m.linear_terms[k][:ref][:coef_var]
        (j[1] > 0.0) ? ub += abs(j[1]) * d[j[2]][end] :
        ub -= abs(j[1]) * d[j[2]][1]
        (j[1] > 0.0) ? lb += abs(j[1]) * d[j[2]][1] :
        lb -= abs(j[1]) * d[j[2]][end]
    end
...
function linear(k, vec)
    return k[:ref][:scalar] .+
           sum([i[1] * vec[i[2]] for i in k[:ref][:coef_var]])
end
...
function detect_discretemulti_term(expr::Any, constr_id::Int, m::Optimizer)
    ...
    if (expr.args[1] == :*)
        ...
        if length(cont_var_idxs) > 1
            ...
            ml_lift_term =
                Alp.detect_nonconvex_terms(ml_term_expr, constr_id, m)
        ...
        if length(bin_var_idxs) > 1
            ...
            bp_lift_term =
                Alp.detect_nonconvex_terms(bp_term_expr, constr_id, m)
        ...
        if length(int_var_idxs) > 1
            ...
            ip_lift_term =
                Alp.detect_nonconvex_terms(ip_term_expr, constr_id, m)
        ...
        if ip_idx < 0 # binlin term if no integer variable
            ...
            return true,
            Alp.lift_nonconvex_term(m, binlin_key, constr_id, scalar)
        end
...
function basic_binlin_bounds(m::Optimizer, k::Any)
    ...
    prod_idxs =
        [i for i in m.nonconvex_terms[k][:var_idxs] if m.var_type[i] == :Cont]
    ...
    if (expr.args[1] == :*)
        ...
        for i in 1:length(expr.args)
            ...
            (expr.args[i].head == :ref) &&
                ...
                push!(var_idxs, expr.args[i].args[2])
            !isempty(var_idxs) &&
                m.var_type[var_idxs[end]] != :Bin &&
                return false, expr
            if (expr.args[i].head == :call)
                down_check, linear_lift_var =
                    Alp.detect_linear_term(expr.args[i], constr_id, m)
                !down_check && return false, expr
                m.var_type[linear_lift_var.args[2]] != :Bin &&
                    return false, expr
                push!(var_idxs, linear_lift_var.args[2])
                continue
            end
...
function detect_bilinear_term(expr::Any, constr_id::Int, m::Optimizer)
    ...
    if (expr.args[1] == :*)  # confirm head (:*)
        ...
        if (length(var_idxs) == 2) && length(Set(var_idxs)) == 2
            term_key =
                [Expr(:ref, :x, var_idxs[1]), Expr(:ref, :x, var_idxs[2])]
            if term_key in keys(m.nonconvex_terms) ||
               reverse(term_key) in keys(m.nonconvex_terms)
                ...
                return true,
                Alp.lift_nonconvex_term(m, term_key, constr_id, scalar)
            else
...
function resolve_convex_constr(
    ...
    if expr_orig == :constr
        ...
        Alp.get_option(m, :log_level) > 99 &&
            println("CONVEX Constraint $(idx): $(expr)")
