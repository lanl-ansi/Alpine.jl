...
function minmax_bound_tightening(
    ...
    (Alp.get_option(m, :log_level) > 0) &&
        println("  Starting bound-tightening")
    ...
    while keep_tightening &&
        ...
        for var_idx in m.candidate_disc_vars
            ...
            if (discretization[var_idx][end] - discretization[var_idx][1]) >
               width_tol
            ...
            if (
                ...
                midpoint =
                    (temp_bounds[var_idx][1] + temp_bounds[var_idx][end]) / 2
            ...
            old_range =
                discretization[var_idx][end] - discretization[var_idx][1]
...
function relaxation_model_obbt(m::Optimizer, discretization, bound::Number)
    ...
    obj_expr = sum(
        m.bounding_obj_mip[:coefs][j] * _index_to_variable_ref(
            m.model_mip,
            m.bounding_obj_mip[:vars][j].args[2],
