...
function create_bounding_mip(m::Optimizer; use_disc = nothing)
    use_disc === nothing ? discretization = m.discretization :
    ...
    m.logs[:time_left] =
        max(0.0, Alp.get_option(m, :time_limit) - m.logs[:total_time])
...
function amp_post_vars(m::Optimizer; kwargs...)
    ...
    if haskey(options, :use_disc)
        l_var = [
            options[:use_disc][i][1] for i in
            1:(m.num_var_orig+m.num_var_linear_mip+m.num_var_nonlinear_mip)
        ]
        u_var = [
            options[:use_disc][i][end] for i in
            1:(m.num_var_orig+m.num_var_linear_mip+m.num_var_nonlinear_mip)
        ]
    else
        l_var = m.l_var_tight
...
function amp_post_lifted_constraints(m::Optimizer)
    for i in 1:m.num_constr_orig
        if m.constr_structure[i] == :affine
            Alp.amp_post_affine_constraint(
                m.model_mip,
                m.bounding_constr_mip[i],
            )
        elseif m.constr_structure[i] == :convex
            Alp.amp_post_convex_constraint(
                m.model_mip,
                m.bounding_constr_mip[i],
            )
        else
...
function amp_post_convex_constraint(model_mip::JuMP.Model, convex::Dict)
    !prod([i == 2 for i in convex[:powers]]) && error(
        "No relaxation implementation for convex constraints $(convex[:expr])",
    )
    ...
    if convex[:sense] == :(<=)
        JuMP.@constraint(
            ...
            sum(
                convex[:coefs][j] *
                _index_to_variable_ref(model_mip, convex[:vars][j].args[2])^2
                for j in 1:convex[:cnt]
            ) <= convex[:rhs]
        )
    elseif convex[:sense] == :(>=)
        JuMP.@constraint(
...
function amp_post_linear_lift_constraints(model_mip::JuMP.Model, l::Dict)
    ...
    JuMP.@constraint(
        ...
        sum(
            i[1] * _index_to_variable_ref(model_mip, i[2]) for
            i in l[:ref][:coef_var]
        ) + l[:ref][:scalar]
    )
    return
    ...
    elseif m.obj_structure == :affine
        JuMP.@objective(
            ...
            m.bounding_obj_mip[:rhs] + sum(
                m.bounding_obj_mip[:coefs][i] * _index_to_variable_ref(
                    m.model_mip,
...
function add_adaptive_partition(m::Optimizer; kwargs...)
    ...
    haskey(options, :branching) ? branching = options[:branching] :
    branching = false

    if length(point_vec) <
       m.num_var_orig + m.num_var_linear_mip + m.num_var_nonlinear_mip
    ...
    for i in m.disc_vars
        ...
        if m.var_type[i] == :Cont
            ...
            for j in 1:λCnt
                if point >= discretization[i][j] &&
                   point <= discretization[i][j+1]  # Locating the right location
                    radius = Alp.calculate_radius(discretization[i], j, ratio)
                    Alp.insert_partition(
                        m,
                        i,
                        j,
                        point,
...
function insert_partition(
    ...
    abstol, reltol = Alp.get_option(m, :disc_abs_width_tol),
    Alp.get_option(m, :disc_rel_width_tol)

    lb_local, ub_local = partvec[partidx], partvec[partidx+1]
    ub_touch, lb_touch = true, true
    lb_new, ub_new =
        max(point - radius, lb_local), min(point + radius, ub_local)
    ...
    if (ub_touch && lb_touch) || Alp.check_solution_history(m, var)
        ...
        point =
            lb_local +
            (ub_local - lb_local) / Alp.get_option(m, :disc_divert_chunks)
        for i in 2:Alp.get_option(m, :disc_divert_chunks)
            insert!(
                partvec,
                pos + 1,
                lb_local +
                chunk * (Alp.get_option(m, :disc_divert_chunks) - (i - 1)),
            )
        end
...
function add_uniform_partition(m::Optimizer; kwargs...)
    ...
    for i in m.disc_vars  # Only construct when discretized
        ...
        chunk =
            distance /
            ((m.logs[:n_iter] + 1) * Alp.get_option(m, :disc_uniform_rate))
        discretization[i] = [
...
function update_disc_ratio(m::Optimizer, presolve = false)
    ...
    for r in ratio_pool
        ...
        Alp.get_option(m, :log_level) > 0 && println(
            "BRANCH RATIO = $(r), METRIC = $(res) || TIME = $(time()-st)",
        )
    end
