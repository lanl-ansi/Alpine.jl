...
function update_disc_cont_var(m::Optimizer)
    ...
    var_diffs = Vector{Float64}(
        undef,
        m.num_var_orig +
        length(keys(m.linear_terms)) +
        length(keys(m.nonconvex_terms)),
    )
...
function heu_pool_multistart(m::Optimizer)
    ...
    for i in 1:m.bound_sol_pool[:cnt]
        if !m.bound_sol_pool[:ubstart][i]
            ...
            l_var, u_var =
                Alp.fix_domains(m, discrete_sol = rounded_sol, use_orig = true)
            heuristic_model = MOI.instantiate(
            ...
            heuristic_model_status =
                MOI.get(heuristic_model, MOI.TerminationStatus())
            if heuristic_model_status in STATUS_OPT ||
               heuristic_model_status in STATUS_LIMIT
                ...
                if eval(convertor[m.sense_orig])(candidate_obj, incumb_obj)
                    ...
                    incumb_sol =
                        round.(
                            MOI.get(heuristic_model, MOI.VariablePrimal(), x),
                            5,
                        )
                    Alp.get_option(m, :log_level) > 0 && println(
                        "Feasible solution obtained using lower bound solution pool [SOL:$(i)] [OBJ=$(incumb_obj)]",
                    )
                end
                found_feasible = true
            else
                Alp.get_option(m, :log_level) > 99 && println(
                    "Multi-start heuristic returns $(heuristic_model_status) [SOL:$(i)]",
                )
            end
