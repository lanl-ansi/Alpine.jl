...
function load!(m::Optimizer)
    ...
    if m.has_nl_objective
        m.obj_expr_orig = Alp.expr_isolate_const(
            _variable_index_to_index(MOI.objective_expr(m.d_orig)),
        ) # see in nlexpr.jl if this expr isolation has any issue
    ...
    (Alp.get_option(m, :nlp_solver) !== nothing) &&
        (Alp.fetch_nlp_solver_identifier(m))
...
function MOI.optimize!(m::Optimizer)
    ...
    if !Alp.check_exit(m) && Alp.get_option(m, :apply_partitioning)
        ...
        Alp.get_option(m, :log_level) > 0 &&
            Alp.logging_row_entry(m, finish_entry = true)
        println(
            "====================================================================================================",
        )
    else
...
function presolve(m::Optimizer)
    ...
    # Solver status
    if m.status[:local_solve] in STATUS_OPT ||
       m.status[:local_solve] in STATUS_LIMIT
    ...
    (Alp.get_option(m, :log_level) > 0) && println(
        "  Completed presolve in $(round.(m.logs[:total_time]; digits = 2))s",
    )
...
function local_solve(m::Optimizer; presolve = false)
    ...
    if presolve == false
        ...
        l_var, u_var =
            m.l_var_tight[1:m.num_var_orig], m.u_var_tight[1:m.num_var_orig]
    end
...
function bounding_solve(m::Optimizer)
    ...
    if status in STATUS_OPT || status in STATUS_LIMIT
        ...
        candidate_bound_sol = [
            round.(
                JuMP.value(_index_to_variable_ref(m.model_mip, i));
                digits = 7,
            ) for i in
