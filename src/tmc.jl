function amp_post_mccormick(m::Optimizer; kwargs...)
    ...
    for bi in keys(m.nonconvex_terms)
        ...
        if (
            (!Alp.get_option(m, :monomial_convexhull)) *
            (nl_type == :MONOMIAL) ||
            (!Alp.get_option(m, :bilinear_convexhull)) * (nl_type == :BILINEAR)
        ) && (m.nonconvex_terms[bi][:convexified] == false)
            @assert length(bi) == 2
            ...
            (lb[idx_a][1] == -Inf) && error(
                "Infinite lower bound detected on VAR$idx_a, resolve it please",
            )
            (lb[idx_b][1] == -Inf) && error(
                "Infinite lower bound detected on VAR$idx_b, resolve it please",
            )
            (ub[idx_a][end] == +Inf) && error(
                "Infinite upper bound detected on VAR$idx_a, resolve it please",
            )
            (ub[idx_b][end] == +Inf) && error(
                "Infinite upper bound detected on VAR$idx_b, resolve it please",
            )

            if (length(lb[idx_a]) == 1) && (length(lb[idx_b]) == 1)  # Basic McCormick
                if m.nonconvex_terms[bi][:nonlinear_type] == :MONOMIAL
                    Alp.mccormick_monomial(
                        m.model_mip,
                        _index_to_variable_ref(m.model_mip, idx_ab),
                        _index_to_variable_ref(m.model_mip, idx_a),
                        lb[idx_a][1],
                        ub[idx_a][1],
                    )
                elseif m.nonconvex_terms[bi][:nonlinear_type] == :BILINEAR
                    Alp.mccormick(
                        ...
                        _index_to_variable_ref(m.model_mip, idx_ab),
                        _index_to_variable_ref(m.model_mip, idx_a),
                        _index_to_variable_ref(m.model_mip, idx_b),
                        lb[idx_a][1],
                        ub[idx_a][1],
                        lb[idx_b][1],
                        ub[idx_b][1],
                ...
                elseif m.nonconvex_terms[bi][:nonlinear_type] == :BILINEAR
                    ...
                    if (idx_a in m.disc_vars) &&
                       !(idx_b in m.disc_vars) &&
                       (part_cnt_b == 1)
                        λ = Alp.amp_post_tmc_λ(
                            m.model_mip,
                            λ,
...
function amp_post_tmc_λ(
    ...
    if !haskey(λ, idx)
        ...
        JuMP.@constraint(
            m,
            _index_to_variable_ref(m, idx) >= dot(lb[idx], λ[idx])
        )
        JuMP.@constraint(
            m,
            _index_to_variable_ref(m, idx) <= dot(ub[idx], λ[idx])
        )
    end
...
function amp_post_tmc_monomial_mc(
    m::JuMP.Model,
    idx_aa::Int,
    λ::Dict,
    λX::Dict,
    ...
    JuMP.@constraint(
        m,
        _index_to_variable_ref(m, idx_aa) >=
        _index_to_variable_ref(m, idx_a)^(2)
    )
    JuMP.@constraint(
        m,
        _index_to_variable_ref(m, idx_aa) <=
        dot(λX[(idx_a, idx_a)], LB[idx_a]) +
        dot(λX[(idx_a, idx_a)], UB[idx_a]) -
        dot(λ[idx_a], *(Matrix(Diagonal(LB[idx_a])), UB[idx_a]))
    )

    return
end

function amp_post_tmc_λX(
    m::JuMP.Model,
    λX::Dict,
    dim::Int,
    idx_a::Int,
    ...
    if !haskey(λX, (idx_a, idx_b))
        λX[(idx_a, idx_b)] = JuMP.@variable(
            m,
            [1:dim],
            base_name = string("L", idx_a, "X", idx_b)
        )
    end
...
function amp_post_tmc_λλ(
    ...
    if !haskey(λλ, (idx_a, idx_b))
        λλ[(idx_a, idx_b)] = JuMP.@variable(
            m,
            [1:dim_a, 1:dim_b],
            base_name = string("L", idx_a, "L", idx_b)
        )
        for i in 1:dim_a
            for j in 1:dim_b
...
function amp_post_tmc_XX_mc(m::JuMP.Model, ab, λX, λλ, LB, UB, a, b)
    ...
    JuMP.@constraint(
        m,
        _index_to_variable_ref(m, ab) .>=
        dot(λX[(a, b)], LB[a]) + dot(λX[(b, a)], LB[b]) -
        reshape(LB[a], (1, dim_A)) * λλ[(a, b)] * reshape(LB[b], (dim_B, 1))
    )
    JuMP.@constraint(
        m,
        _index_to_variable_ref(m, ab) .>=
        dot(λX[(a, b)], UB[a]) + dot(λX[(b, a)], UB[b]) -
        reshape(UB[a], (1, dim_A)) * λλ[(a, b)] * reshape(UB[b], (dim_B, 1))
    )
    JuMP.@constraint(
        m,
        _index_to_variable_ref(m, ab) .<=
        dot(λX[(a, b)], LB[a]) + dot(λX[(b, a)], UB[b]) -
        reshape(LB[a], (1, dim_A)) * λλ[(a, b)] * reshape(UB[b], (dim_B, 1))
    )
    JuMP.@constraint(
        m,
        _index_to_variable_ref(m, ab) .<=
        dot(λX[(a, b)], UB[a]) + dot(λX[(b, a)], LB[b]) -
        reshape(UB[a], (1, dim_A)) * λλ[(a, b)] * reshape(LB[b], (dim_B, 1))
    )
...
function mccormick_binlin(
    m::JuMP.Model,
    binlin::JuMP.VariableRef,
    bin::JuMP.VariableRef,
    lin::JuMP.VariableRef,
    lb,
    ub,
)
