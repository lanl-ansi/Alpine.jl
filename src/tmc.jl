function amp_post_mccormick(m::Optimizer; kwargs...)
    ...
    for bi in keys(m.nonconvex_terms)
        ...
        if (
            (!Alp.get_option(m, :monomial_convexhull)) * (nl_type == :MONOMIAL) ||
            ...
            (lb[idx_a][1] == -Inf) &&
                error("Infinite lower bound detected on VAR$idx_a, resolve it please")
            (lb[idx_b][1] == -Inf) &&
                error("Infinite lower bound detected on VAR$idx_b, resolve it please")
            (ub[idx_a][end] == +Inf) &&
                error("Infinite upper bound detected on VAR$idx_a, resolve it please")
            (ub[idx_b][end] == +Inf) &&
                error("Infinite upper bound detected on VAR$idx_b, resolve it please")
                ...
                elseif m.nonconvex_terms[bi][:nonlinear_type] == :BILINEAR
                    ...
                    if (idx_a in m.disc_vars) &&
                       ...
                       (part_cnt_b == 1)
                        λ = Alp.amp_post_tmc_λ(m.model_mip, λ, lb, ub, part_cnt_a, idx_a)
                        λX =
                            Alp.amp_post_tmc_λX(m.model_mip, λX, part_cnt_a, idx_a, idx_b)
                    ...
                    if !(idx_a in m.disc_vars) &&
                       ...
                       (part_cnt_a == 1)
                        ...
                        λX =
                            Alp.amp_post_tmc_λX(m.model_mip, λX, part_cnt_b, idx_b, idx_a)
...
function amp_post_tmc_monomial_mc(
    ...
    JuMP.@constraint(
        m,
        _index_to_variable_ref(m, idx_aa) >= _index_to_variable_ref(m, idx_a)^(2)
    )
    JuMP.@constraint(
        m,
        _index_to_variable_ref(m, idx_aa) <=
        dot(λX[(idx_a, idx_a)], LB[idx_a]) + dot(λX[(idx_a, idx_a)], UB[idx_a]) -
...
function amp_post_tmc_λX(m::JuMP.Model, λX::Dict, dim::Int, idx_a::Int, idx_b::Int)
    if !haskey(λX, (idx_a, idx_b))
        λX[(idx_a, idx_b)] =
            JuMP.@variable(m, [1:dim], base_name = string("L", idx_a, "X", idx_b))
    end
